    ...
    export async function addNewUpdate(req: Request, res: Response) { //send user id as user_id
        ...
        try {
            ...
            if (MAX_AWAY_DISTANCE_FROM_TRACK < dh.height) {
                return res.send({success: false, message: "little far away from track"});
            }
            ...
            if (previousUserPoint && previousUserPoint!.line_id === userPoint.line_id) {
                ...
                userPoint.acceleration = va.acceleration;
                appLogger.info("PVA p:" + userPoint.position+ ",V:"+userPoint.velocity+",A:"+userPoint.acceleration);
            } else {
            ...
            const msg = previousUserPoint ? getMessage(userPoint, previousUserPoint) : "initial entry";
    ...
    function getMessage(userPoint: UserPoint, previousUserPoint: UserPoint) {
        if (0 < userPoint.train_id) {
            if (previousUserPoint.train_id == 0) {
                return "new train created with id: " + userPoint.train_id;
            } else if (userPoint.train_id !== previousUserPoint.train_id) {
                return "train changed to id: " + userPoint.train_id;
            } else {
                return "train id: " + userPoint.train_id;
            }
        } else {
            return "continuation count: " + userPoint.continuation;
        }
    }
}
