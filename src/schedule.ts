...
class Schedule {
    ...
    private static async updateExistingTrains(timestamp: number) {
        ...
        for (let train of trains) {
            ...
            const trainUserPoints5s = await LocationDao.findUsersForTrain(train._id, timestamp - (5 * 1000));
            if (0 < trainUserPoints5s.length) {
                const dvas = trainUserPoints5s.map((p) => {
                ...
                appLogger.info("TRAIN PVA p:" + train.position + ",V:" + train.velocity + ",A:" + train.acceleration);
                await train.save();
            }

            const trainUserPoints80s = await LocationDao.findUsersForTrain(train._id, timestamp - (80 * 1000));
            if (trainUserPoints80s.length === 0) {
